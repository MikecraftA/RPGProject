...
namespace Ge
{
    ...
    public class Chunk
    {
        ...
        private Vector3 GenerateBlock(int i, int j, int seed)
        {
            ...
            float ocean = 0;// NoiseLib.ExpNoise(SIZE * x + i, SIZE * y + j, seed, 1000, 1 * Mathf.PerlinNoise((SIZE * x + i + seed + 2) / 60f, (SIZE * y + j + seed + 2) / 60f), 1.0f, 1.0f, 0.5f);
    ...
    public static class NoiseLib
    {
        ...
        // Given a seed, random.Next(a constant) always gives you a same value
        // For more info just Google you lazy ass
